...
> ※1 SHIB: 수정 전에는 K/D 선택 봉이 달라 5m K ≥ 40 조건을 항상 충족해 진입 불가했으나,
> 수정 후 close_time 기반으로 바뀌며 조건 충족 시점이 달라져 진입 발생.
> 이것은 편향 제거의 결과이며 의도된 변화입니다.

**수정 후 C-4 기준선 (새 비교 기준)**:
BONK 63건 / -96.61% / MDD 131.13% / PF 0.611 | SHIB 33건 / +12.05% / MDD 44.61% / PF 1.137
...
> ※2 SHIB/PEPE: API 레이트 제한으로 5m/15m 봉 데이터 로딩 실패 → 결과 신뢰 불가.
> BONK 결과만 신뢰 가능.
...
수정 후 BONK 단독(15건, min_n=15) → 3비BTC 합계 60건·심볼당 15건 조건 미달.
**C-5 SLOPE=5 DIV=None — 재판정 보류** (SHIB/PEPE 데이터 부족으로 완전한 판정 불가).

> ⚠️ 현재 _strategy.json slope=5.0·div=null 설정은 "검증된 설정"으로 취급하지 말 것.
> ⚠️ 완전한 재측정(SHIB/PEPE 포함)이 필요합니다.
