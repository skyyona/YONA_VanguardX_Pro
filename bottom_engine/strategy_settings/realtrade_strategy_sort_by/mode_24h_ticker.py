...
[적용 대상]
  ...
  실제 진입은 ATR 임계값(1h ATR ≥ 1.65%)을 초과하는 구간에서만 발생한다.

[ATR 임계값 구조 — G4·G8 두 게이트가 독립적으로 동일한 벽을 형성]
  G4 (long/short_condition.py):
    _atr_min_eff = max(cfg.atr_min, _sl_used / 2.0) = max(0.10, 3.3/2.0) = 1.65%
    → 1h ATR < 1.65% 이면 G4에서 직접 차단

  G8 common_liq (prohibition_filter.py):
    liq_long_pct = f(ATR × 2) — 1h ATR 기반 청산 거리 추정값
    → ATR × 2 < sl_used(3.3) → ATR < 1.65% 이면 G8에서도 차단
...
[정상 동작 범위]
  ...
  고변동성 알트 (DOGE 등): 1h ATR 중앙값 0.641% (41일 실측) → 평상시 차단,
    급등·급락 구간 (ATR > 1.65%) 에서만 진입 — 이것이 의도된 동작이다.
